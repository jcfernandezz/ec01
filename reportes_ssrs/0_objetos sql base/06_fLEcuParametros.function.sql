IF OBJECT_ID ('dbo.fLEcuParametros') IS NOT NULL
   DROP FUNCTION dbo.fLEcuParametros
GO

create function dbo.fLEcuParametros(@tag1 varchar(15), @tag2 varchar(15), @tag3 varchar(15), @tag4 varchar(15), @tag5 varchar(15), @tag6 varchar(15))
returns table
as
--Propósito. Devuelve los parámetros de la compañía para localización Ecuador. 
...
(
	...
	and ia.ADRSCODE = 'LECU'
)
go


IF (@@Error = 0) PRINT 'Creación exitosa de la función: fLEcuParametros()'
ELSE PRINT 'Error en la creación de la función: fLEcuParametros()'
GO
-------------------------------------------------------------------------------
--select PARAM1
--from fLEcuParametros('NAUT', '-', '-', '-', '-', '-')
